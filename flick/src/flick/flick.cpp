...
void Flick::addToPathPermanent(const std::vector<std::string>& paths) {
  ...
  if (checkFile.is_open()) {
    existingContent.assign((std::istreambuf_iterator<char>(checkFile)),
...
std::pair<ProjectType, Language> Flick::readuserInput() {
  Log::log("Choose language: \033[95mc / cc \033[0m(default = cc), q = quit > ",
           Type::E_DISPLAY, "");
  ...
  Log::log(
      "Choose project type:\033[95m x = executable, l = library \033[0m(Enter "
      "to cancel) > ",
      Type::E_DISPLAY, "");
...
Flick::Flick(const std::vector<std::string>& args) {
  ...
  if (cmd == "new") {
    if (args.size() < 3) {
      Log::log(
          "Forgot to name your project? Even \033[95m‘untitled’\033[0m "
          "would’ve worked",
          Type::E_ERROR);
      exit(0);
    };
    ...
    // if (std::any_of(project_name.begin(), project_name.end(), [](const char
    // &c) 				{ return !((c >= 'a' && c <= 'z') ||
    // isdigit(c) || c == '-'); }))
    if (!std::regex_match(projectName, pattern)) {
      Log::log(
          "Project name must be in lowercase with no space and special "
          "characters allowed name : \033[95m^[a-z][a-z0-9-]*$\033[0m",
          Type::E_ERROR);
      std::exit(0);
    };
...
bool Flick::executeCMake(const std::vector<std::string>& additionalCMakeArg) {
  ...
  std::vector<std::string> args{"cmake", "-S",   ".", "-DENABLE_TESTS=OFF",
                                "-G",    "Ninja"};
  ...
  return ProcessManager::startProcess(
             args, processLog,
             "Generating CMake Files this may take a while") == 0;
};
...
bool Flick::compile() {
  ...
  // auto formatedString = fmt::format("Threads in use : {}",
  // cpuThreads.c_str()); Log::log(formatedString, Type::E_DISPLAY);
  for (auto& arg : mArgs) {
  ...
  if (!fs::exists(fs::current_path().string() + "/build/" + VCPKG_TRIPLET)) {
    // run cmake
    std::vector<std::string> args{"-DCMAKE_BUILD_TYPE=Debug",
                                  "--preset=" + std::string(VCPKG_TRIPLET)};
    ...
    if (ProcessManager::startProcess(args, pLog,
...
void Flick::run() {
  ...
  if (!fs::exists(run)) {
    Log::log("Subproject \033[95m'" + app +
                 "'\033[0m has no executable. Try verifying the name and "
  ...
  // Log::log(fmt::format("Running
  // \033[95m{}\033[0m\n────────────────────────────────────────────────────────────",
  // app), Type::E_DISPLAY);
  c.wait();
  // fmt::println("────────────────────────────────────────────────────────────");
  Log::log(
      fmt::format("\033[95m{}\033[0m exited with code {}", app, c.exit_code()),
      Type::E_DISPLAY);
}
...
bool addToPathPermanentWindows(const std::string& newPath) {
  ...
  if (RegOpenKeyExA(HKEY_CURRENT_USER, envKey, 0, KEY_READ | KEY_WRITE,
                    &hKey) != ERROR_SUCCESS) {
  ...
  if (!updatedPath.empty() && updatedPath.back() != ';') updatedPath += ';';
  ...
  if (RegSetValueExA(hKey, pathName, 0, REG_EXPAND_SZ,
                     ...
                     static_cast<DWORD>(updatedPath.size() + 1)) !=
      ERROR_SUCCESS) {
  ...
  Log::log(
      fmt::format("Added to PATH permanently: \033[95m{}\033[0m", newPath));
  return true;
}
...
void Flick::addToPathWin() {
  ...
  if (source != destination) {
    try {
      ...
      Log::log(fmt::format("Failed to create flick directory: {}", e.what()),
               Type::E_ERROR);
      return;
    }
    ...
    for (const auto& dll : fs::directory_iterator(fs::current_path())) {
      ...
      if (dll.path().extension() == ".dll") {
        ...
        try {
          Log::log(fmt::format("Copying {} to {}",
                               dll.path().filename().string().c_str(),
                               flickDir.c_str()));
          fs::copy_file(dll.path(), dllDest, fs::copy_options::update_existing);
        } catch (const std::exception& e) {
          Log::log(fmt::format("Failed to copy {}: {}",
                               dll.path().filename().string(), e.what()),
                   Type::E_WARNING);
        }
      }
    }
    ...
    if (!fs::exists(source)) {
      ...
      try {
        ...
        Log::log(fmt::format("Error copying Flick.exe: {}", e.what()),
                 Type::E_ERROR);
      }
    }
  }
  ...
  for (const auto& dir : fs::directory_iterator(flickDir)) {
    ...
    if (pathStr.find("_internal") != std::string::npos) continue;
  ...
  if (allFound) {
    ...
    Log::log(
        "Some Flick paths are missing from PATH. Adding them permanently. "
...
void Flick::addToPathUnix() {
  ...
  if (source != destination) {
    ...
    if (fs::copy_file(source, destination,
      ...
      Log::log(
          fmt::format("{} copied to {}", source.c_str(), destination.c_str()));
    } else {
      Log::log("Error while copying Flick into flick directory!",
               Type::E_ERROR);
      return;
    }
  }
  ...
  for (const auto& dir : fs::directory_iterator(flickDir)) {
    if (dir.is_directory()) {
      ...
      if (dirPath.find("_internal") != std::string::npos) continue;
  ...
  if (missingPaths.empty()) {
    ...
    Log::log(
        "Some paths from Flick are missing in PATH. Adding permanently to "
        "~/.bashrc",
        Type::E_WARNING);
...
void Flick::setupVcpkg(const std::string& home, bool& is_install) {
  ...
  try {
    std::vector<std::string> args{"git", "clone",
                                  "https://github.com/microsoft/vcpkg.git",
                                  std::string(home) + "/vcpkg"};
    if (ProcessManager::startProcess(
            args, processLog, "Installing VCPKG From Github", false) != 0) {
...
void Flick::installTools(bool& isInstallationComplete) {
  ...
  namespace fs = std::filesystem;
  Log::log(
      "This will install Clang-LLVM Toolchain with cmake,  ninja and vcpkg "
      "package manager from Github,\nAre you sure you "
      "want to "
      "continue??[y/n]",
      Type::E_DISPLAY);
  char c;
  ...
  if (tolower(c) != 'y') exit(0);
  std::string home = getenv(USERNAME);
  if (!home.c_str()) return;
  home += "\\flick";
  // system(("start " + std::string(VS_URL)).c_str());
  // Log::log("Make sure you download Desktop Development in C++ from Visual
  // Studio Installer", Type::E_WARNING);
  std::string processLog{};
  if (ProcessManager::startProcess({"clang", "--version"}, processLog,
                                   "Checking Clang Compiler") != 0) {
  ...
  if (ProcessManager::startProcess({"cmake", "--version"}, processLog,
                                   "Checking CMake") != 0) {
    Downloader::download(std::string(CMAKE_URL), home + "\\cmake.zip");
    Unzip::unzip(home + "\\cmake.zip", home);
    fs::remove((home + "\\cmake.zip"));
  }
  if (ProcessManager::startProcess({"ninja", "--version"}, processLog,
                                   "Checking Clang Compiler") != 0) {
    Downloader::download(std::string(NINJA_URL), home + "\\ninja.zip");
    Unzip::unzip(home + "\\ninja.zip", home);
    fs::remove((home + "\\ninja.zip"));
  }
  if (!fs::exists(fs::path("C:\\Program Files (x86)\\Microsoft Visual "
                           "Studio\\Installer\\vswhere.exe"))) {
    Downloader::download(std::string(VS_BUILD_TOOLS_INSTALLER_URL),
                         home + "\\vs.exe");
    Downloader::download(
        "https://github.com/vishal-ahirwar/flick/blob/master/res/"
        "flick.vsconfig",
        home + "\\flick.vsconfig");
    if (system((home + "\\vs.exe --quiet --wait --config " + home +
                "\\flick.vsconfig")
                   .c_str())) {
      Log::log("installing Visual Studio C++ Build Tools failed!",
               Type::E_ERROR);
    } else {
      Log::log("Visual Studio C++ Build Tools has been installed!",
               Type::E_DISPLAY);
      fs::remove((home + "\\vs.exe"));
    };
  } else {
    Log::log(
        "Visual Studio Installer already installed on your system install "
        "buildtools now",
        Type::E_WARNING);
  }
...
#define DISTRO_INFO "/etc/os-release"
  ...
  Log::log(
      "Install C++ clang Compiler and build tools using ex.[ubuntu]sudo apt "
      "install git ninja-build cmake clang clang-tools",
      Type::E_WARNING);
  addToPathUnix();
...
#else
  Log::log(
      "[Attention] On Linux in order to use vcpkg packages without any issue "
      "please install pkgcofig, clang, lldb",
      Type::E_WARNING);
#endif  // _WIN32
};
...
void Flick::createInstaller() {
  ...
  std::vector<std::string> args{"-Bbuild/release", "-DCMAKE_BUILD_TYPE=Release",
                                "--preset=" + std::string(VCPKG_TRIPLET)};
  if (executeCMake(args)) {
  ...
  if (!system("cd build/release && cpack")) return;
  std::ofstream file;
...
void Flick::test() {
  ...
  if (!tester.runUnitTesting(mArgs)) return;
#ifdef _WIN32
...
bool Flick::onSetup() {
  ...
  if (file.is_open()) {
    ...
    if (isInstallationComplete) {
      Log::log(
          "Compiler is already installed if you think you messed up with Flick "
          "installation please use this command Flick fix",
          Type::E_WARNING);
      return true;
    }
  } else {
...
// TODO
// remove the ~/Flick and reinstall the Flick again with all the tools like
// cmake,g++ compiler,ninja,nsis
...
// TODO
  ...
  for (const auto& dir : fs::directory_iterator(path)) {
    if (dir.is_directory()) {
      fs::remove_all(dir);
      Log::log(fmt::format("\033[95m{}\033[0m removed.", dir.path().string()),
               Type::E_WARNING);
    }
  }
  bool status{};
...
void createProcess(const std::string& path) {
  ...
  if (pid == 0) {
    ...
    // Parent process - can optionally wait for the child to finish or log the
    // success
    Log::log("Updater started successfully in the background.",
             Type::E_DISPLAY);
    return;
  }
#endif
}
// download the utool from github if it's not already there then download the
...
void Flick::update() {
  ...
  if (fs::exists(source))  // if utool is present in ~/Flick directory then
                           // start the utool if not download the utool first
  {
    createProcess(source);  // Windows//starting process parent-less which will
                            // start utool so Flick will shutdown and then utool
                            // override the Flick.exe
  } else {
    Log::log("Could not find updater tool,you have to re-install the flick!",
             Type::E_ERROR);
  };
};
// TODO
void Flick::debug() {
  if (!compile()) return;
#ifdef _WIN32
  system(("lldb ./build/debug/" + mProjectSetting.getProjectName() + ".exe")
             .c_str());
#else
...
bool Flick::release() {
  ...
  if (!fs::exists(fs::current_path().string() + "/build/release")) {
    std::vector<std::string> args{"-Bbuild/release",
...
// everytime user run this command it's will override everything in
...
void Flick::addDeps() {
  ...
  if (name.empty()) {
    Log::log(
        "Uses : flick install \033[95m--package=boost-process --version=1.85.0 "
        "--project=demo --update-base-line\033[0m",
        Type::E_ERROR);
    Log::log(
        "Other args are optional but package name must be provided: flick "
        "install \033[95m--package=fmt\033[0m",
        Type::E_ERROR);
    return;
  }
  if (bUseLatest) {
    Log::log("fetching latest packages from \033[95mvcpkg\033[0m",
             Type::E_WARNING);
    std::string log{};
...
void Flick::genCMakePreset() {
  Log::log(
      "Please Choose your Programming language \033[95mc/cc\033[0m "
      "default=cc,q=quit > ",
      Type::E_DISPLAY, "");
  std::string input{};
  std::getline(std::cin, input);
...
void Flick::createSubProject() {
  if (mArgs.size() < 3) {
    Log::log(
        "No subproject name? What do you want me to do, \033[95mguess\033[0m?",
        Type::E_ERROR);
    return;
  }
  auto info = readuserInput();
...
void Flick::listPackages() {
  ...
  if (in.is_open()) {
    ...
    Log::log(fmt::format("total dependencies: \033[95m{}\033[0m",
                         dependencies.size()));
    for (const auto& dep : dependencies) {
      Log::log(
          fmt::format("\033[95m------{} \033[0mversion : {}",
                      dep.get<std::string>(),
                      [&j](const std::string& dep) -> std::string {
                        ...
                        for (const auto& override : overrides) {
                          if (override.contains("name") &&
                              override["name"].get<std::string>() == dep) {
                            return override.contains("version")
                                       ? override["version"].get<std::string>()
                                       : "latest";
                          };
                        };
                        return "latest";
