...
int ProcessManager::startProcess(const std::vector<std::string>& args,
                                 std::string& processLog,
                                 const std::string& msg, bool b_log) {
  std::shared_ptr<barkeep::AnimationDisplay> anim{};
  if (!msg.empty()) {
    if (!b_log)
      anim = bk::Animation({.message = "\033[32m[+]\033[0m " + msg,
  ...
  boost::process::child process(boost::process::search_path(args[0]),
                                boost::process::args(std::vector<std::string>(
                                    args.begin() + 1, args.end())),
                                boost::process::std_out > outStream,
                                boost::process::std_err > errStream);
  std::string lineOut, lineErr;
  while (process.running() && (std::getline(outStream, lineOut) ||
                               std::getline(errStream, lineErr))) {
    if (!lineErr.empty()) {
  ...
  if (!fs.is_open()) {
    ...
    if (exitCode != 0) {
      Log::log("You can find more info in \033[95mbuild/logs.txt\033[0m",
               Type::E_DISPLAY);
    }
    fs.close();
  }
